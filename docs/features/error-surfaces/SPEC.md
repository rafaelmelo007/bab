...
**Source:** docs/prds/2026-05-23-bab.md §6 — F-08; §4.8, §7.2 (log redaction), §4.7 (NO_COLOR + ASCII)
...
Every failure surface in bab — transport (F-02), session-management (F-05), slash-commands (F-06), one-shot (F-07), state-store (F-04) — must format errors the same way so a user can grep, alias, and recognize them across modes. PRD §4.8 catalogs 13 codes (`E-CLI-MISSING`, `E-ACP-PROTOCOL`, `E-STATE-LOCKED`, …) with fixed message templates. This feature delivers a small TypeScript module: a `BabError` class hierarchy covering all 13 codes, an ANSI-aware formatter honoring `NO_COLOR` and TTY detection, the credential-redaction patterns from §7.2 (Bearer, Authorization, OAuth refresh URLs, Basic auth), exit-code mapping for one-shot mode, and a CI-tested negative corpus that keeps the redactor's false-positive rate under 1%. The PRD contract is that **errors never crash the REPL** — they return to the prompt.

## §2 Scope

### In Scope
- `BabError` class hierarchy (or discriminated-union type) covering all 13 §4.8 codes; `format(opts)` method producing `✗ <CODE>: <human message>` with ASCII fallback (`[err] <CODE>: <msg>`) per §4.7.
...
- Credential redaction module (`src/redact/index.ts`) with the four default patterns from §7.2; additive-only `BAB_REDACT_EXTRA` env extension (no way to disable defaults); pattern version constant `REDACT_VERSION` bumped per change.
...
- Defense-in-depth: bab's own stderr (not only subprocess stderr) is routed through the redactor.
- INTERFACE-CONTRACTS.md pins the `BabError` shape so F-02/F-05/F-06/F-07 can lock against it.
...
- Structured logging — bab does not log to a file by default in v1.
...
- [ ] AC-01: `new BabError.CliMissing(provider, docUrl).format()` renders exactly `✗ E-CLI-MISSING: provider 'X': not installed. Install: <doc URL>` on TTY+UTF-8+color; one-shot exit code 127 (per F-07 D-01).
...
- [ ] AC-04: `CliTimeout(provider, seconds)` renders exactly `✗ E-CLI-TIMEOUT: provider 'X' timed out after Ns. Set BAB_TURN_TIMEOUT to extend.`; one-shot exit 124.
...
- [ ] AC-09: `StateLocked` coalesces retries — prints `another bab process is writing state. Retrying...` once on first contention, then `E-STATE-LOCK-FAILED` if all 3 attempts fail (per D-02); final exit 74.
- [ ] AC-10: `StateCorrupt(path)` renders exactly `✗ E-STATE-CORRUPT: <path> is corrupt. Move it aside and re-run.`; exit 74.
- [ ] AC-11: `StateSchema(found, supported)` renders exactly `✗ E-STATE-SCHEMA: state.toml is from a newer bab (vN, this is vM). Upgrade bab or use BAB_CONFIG_DIR.`; exit 74.
...
- [ ] AC-21: `BAB_REDACT_EXTRA="<r1>,<r2>"` adds patterns on top of defaults (additive only — no way to disable defaults per D-03); invalid regex warns at startup but defaults still apply.
...
- [ ] AC-24: `babError.exitCode()` returns the table mapping documented above; mapping owned by F-07 D-01 and consumed here; table-tested.
...
| Security | Default patterns: `Bearer …`, `Authorization: .*`, `https://…[?&](access_token\|refresh_token)=`, `Basic [A-Za-z0-9+/=]{20,}`. `BAB_REDACT_EXTRA` additive only (D-03). Pattern version recorded in `REDACT_VERSION` constant; logged in telemetry's `error_<code>` event (version int only, never the patterns themselves). Defense-in-depth: bab's own stderr also routed through the redactor (D-05). |
...
| TC-06 | Unit | `BAB_REDACT_EXTRA` adds patterns; invalid regex warns | Startup test with valid + invalid inputs |
...
| TC-10 | Integration | One-shot exits with documented exit code per variant | Spawn `bab -p invalid` etc. via `node:child_process`, assert `code` callback value |
...
- **PRD:** docs/prds/2026-05-23-bab.md §6 — F-08; §4.8, §7.2
...
- F-08 ships first — every other feature (F-02, F-05, F-06, F-07) depends on the `BabError` shape but not on its full body. Keep the public surface (pinned in INTERFACE-CONTRACTS.md) stable from week 1; the redaction module can iterate.
