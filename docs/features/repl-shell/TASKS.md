...
| T-02 | Implement `renderPrompt(state)` pure function: `"ulm> "` when no provider, `"ulm (<name>)> "` when set; no ANSI under NO_COLOR / non-TTY | backend-lead | High | Done | AC-01, AC-10 | — |
...
| T-12 | Write vitest unit test for `renderPrompt(undefined)` / `renderPrompt("claude")` returning `"ulm> "` / `"ulm (claude)> "` exactly | testing-lead | High | Done | TC-01 | — |
...
| T-23 | Wire `tinybench` startup benchmark: 1000 iterations of `node bin/ulm.mjs </dev/null` ≤ 80 ms p95 on `ubuntu-latest` | perf-specialist | High | Done | TC-11, AC-12 | — |
