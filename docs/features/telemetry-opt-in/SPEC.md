...
**Source:** docs/prds/2026-05-23-ulm.md §6 — F-10; §10.1, §10.2, §10.3
...
The PRD's founder-conviction bet (§3.1) is falsified or validated by §10.1 adoption signals — first-week retention, mean providers used per installation, crash-free run rate. None of those can be measured from GitHub stars or package-registry numbers alone; they require opt-in instrumentation inside running ulm installs. This feature implements the `ulm telemetry enable|disable|local` subcommand, the §10.2 event allowlist, the anonymous 128-bit install ID, the async POST to the telemetry endpoint with local-cache spool-and-retry, and the `ULM_NO_TELEMETRY=1` kill switch. It must never block the REPL, never send prompt or session content, and never call out without an explicit opt-in.

## §2 Scope

### In Scope
- `ulm telemetry enable` — generates 128-bit anon ID via `node:crypto.randomBytes(16).toString('hex')` (D-01); prints verbatim "what is/isn't sent" text contract; persists `[telemetry]` block via F-04.
- `ulm telemetry disable` — purges cache + sets `enabled = false` + `mode = "off"` + zeroes `anon_id` (D-07 secure overwrite).
- `ulm telemetry local` — sets `mode = "local"`; events append to `$ULM_CACHE_DIR/telemetry.jsonl` only (no HTTP sender).
...
- Per-event allowed fields: `timestamp` (RFC3339 UTC ms precision), `anon_id`, `os`, `ulm_version`, `event_name`, `event_data` (strict allowlist tested in `tests/telemetry/`).
- Endpoint: `https://telemetry.ulm.<TBD per Q-08>/v1/event` — payload schema in [INTERFACE-CONTRACTS.md](./INTERFACE-CONTRACTS.md).
...
- Local-cache file at `$ULM_CACHE_DIR/telemetry.jsonl`, mode 0600, 5 MB cap with FIFO trim (D-03).
- Kill switches: `ULM_NO_TELEMETRY=1` overrides config; `ulm telemetry disable` overrides config.
...
- [ ] AC-01: `ulm telemetry enable` writes `[telemetry].enabled = true`, `mode = "remote"`, generates `anon_id` as 32-char lowercase hex from `node:crypto.randomBytes(16).toString('hex')`.
- [ ] AC-02: `ulm telemetry enable` prints, verbatim, the §4 "what is/isn't sent" text contract before persisting opt-in. Exact string fixture-tested in `tests/telemetry/enable_output.txt`.
- [ ] AC-03: `ulm telemetry enable` exits non-zero and persists nothing if the user does not type `yes` (case-insensitive) at the confirmation prompt; the verbatim list is shown before the prompt, never after persistence.
- [ ] AC-04: `ulm telemetry disable` sets `enabled = false`, deletes `$ULM_CACHE_DIR/telemetry.jsonl` and any in-flight retry queue, and zeroes `anon_id` to empty string in state.toml. Subsequent `enable` generates a *new* anon_id.
- [ ] AC-05: `ulm telemetry local` sets `mode = "local"`; emitted events append to `$ULM_CACHE_DIR/telemetry.jsonl` only — HTTP sender not started.
...
- [ ] AC-07: `ULM_NO_TELEMETRY=1` (any non-empty value other than `0` / `false`) overrides config: no emit, no local-cache write, no socket open — even with `enabled = true` on disk. Tested with both `mode = "remote"` and `mode = "local"`.
...
- [ ] AC-13: On HTTP send failure (non-2xx, timeout > 2 s, DNS, connect refused), the event is appended to `$ULM_CACHE_DIR/telemetry.jsonl`. Per-event retry: 3 attempts with backoff 250 ms / 1 s / 4 s; after the third, the event remains in the cache for the drain loop. No event drops on first failure.
...
- [ ] AC-16: `anon_id` never appears in any log file, worklog entry, or stderr output. Scanned in integration tests. Anon ID rotation on `ulm reset` is irrevocable: old ID overwritten via `buf.fill(0)` before state.toml rewrite (D-07).
...
| Accessibility | `ulm telemetry enable` "what is/isn't sent" list is plain English; respects NO_COLOR. |
| Security | No prompt/session/path/env content ever transmitted; allowlist tested with negative-corpus in `tests/telemetry/`; 128-bit anon ID generated with `node:crypto.randomBytes(16)`; HTTPS-only with TLS 1.2+; build-time-constant endpoint URL (D-04 — no runtime exfiltration). Anon-ID rotation overwrites old value with `buf.fill(0)` before rewrite (D-07). ulm's parent process (which spawns provider subprocesses) inherits the original denylisted env via F-02 D-02, so no env→event_data leakage path exists. |
...
> Opt-in flag, anon ID, and tri-state mode live in state.toml under `[telemetry]` (owned by F-04, F-10 writes per F-04 reserved-section contract). Local-cache file `$ULM_CACHE_DIR/telemetry.jsonl` is owned by this feature; format in INTERFACE-CONTRACTS.md.
...
| TC-05 | Unit | Verbatim "what is/isn't sent" string | Output of `ulm telemetry enable` matches `tests/telemetry/enable_output.txt` byte-for-byte (line-ending-normalized) |
| TC-06 | Unit | Kill switch | `ULM_NO_TELEMETRY=1` short-circuits emit before allowlist check; no JSONL write, no socket open |
| TC-07 | Integration | Opt-in flow | `ulm telemetry enable` → `yes` → state.toml mutated atomically via F-04 contract; mode = "remote"; anon_id set |
| TC-08 | Integration | Disable purges cache | Pre-seed 100 events in JSONL → `ulm telemetry disable` → file absent; subsequent enable creates new anon_id |
...
- **PRD:** docs/prds/2026-05-23-ulm.md §6 — F-10; §10.1, §10.2, §10.3
...
|---|----------|-------|--------|------------|
| Q-01 | User-visible behavior when `$ULM_CACHE_DIR` is unwritable | backend-lead | Open | Decision criterion: refuse-and-error vs degrade-to-in-memory-only. Tied to AC-15. Target 2026-06-30. |
...
- The local-cache file is documented as user-readable but never shipped (per §7.2). `ulm telemetry local` is the discoverable inspection path.
- The "what is/isn't sent" list printed by `ulm telemetry enable` is a verbatim contract — changes require a SPEC update and a fixture refresh.
- D-04 build-time-constant URL: defined as `export const TELEMETRY_URL: \`https://${string}\` = "https://telemetry.ulm..."` in `src/telemetry/endpoint.ts`. No env-var override path. Audit by `grep TELEMETRY_URL` in the published bundle.
