...
**Source:** docs/prds/2026-05-23-bab.md §6 — F-07; §4.6, §4.7, §4.8, §5.2.1, §7.2
...
- Hard wall-clock cap = `BAB_TURN_TIMEOUT + 2s` after which bab itself exits `CliTimeout` (never hangs).

### Out of Scope
- The REPL loop — F-01. Verified by AC-12.
- Multi-turn conversation in one-shot — single round-trip per §4.6.
- Piping between providers (`bab -p claude "..." | bab -p gemini "..."` works at shell level; no special `/pipe` — v2 candidate, DEF-03).
- `bab telemetry ...` / `bab daemon ...` subcommand surfaces — share the commander parser but owned by F-10 / future daemon.
...
- [ ] AC-01: Positional + `-p` → `bab -p claude "summarize this"` invokes Claude transport with prompt verbatim; streams to stdout; exits 0 on success.
- [ ] AC-02: Stdin-piped — `echo "rewrite in french" | bab -p gemini` reads stdin to EOF; trailing newline stripped; Gemini transport invoked; exits 0.
- [ ] AC-03: Positional + stdin concatenation per D-03 — `bab -p claude "summarize this" < notes.txt` → transport receives positional + `\n\n` + stdin (trailing newline stripped).
- [ ] AC-04: `--` separator (§7.2) — `bab -p claude -- --help me write a function` treats `"--help me write a function"` as the prompt; no usage text printed.
...
- [ ] AC-07: ANSI auto-off — `bab -p claude "hi" | cat > out.txt` → `out.txt` contains zero ANSI escapes; `NO_COLOR=1 bab -p claude "hi"` on a TTY also produces ANSI-free output; `FORCE_COLOR=1 bab -p claude "hi" | cat` retains ANSI.
...
- [ ] AC-09: No shell interpretation — `bab -p mock-provider "$(rm -rf /tmp/canary)"` with canary pre-created → canary intact. Provider receives literal `$(rm -rf /tmp/canary)`. CI grep from §7.2 confirms no `spawn(..., { shell: true })` in `src/oneshot/`.
- [ ] AC-10: Exit-code mapping per D-01 — table-driven: mock exits 1 → bab exits with `CliCrash` mapping (1); `bab -p unknown "x"` → `InvalidProvider` mapping (2); claude not on PATH → `CliMissing` mapping (127); `BAB_TURN_TIMEOUT=1` + hung mock → `CliTimeout` mapping (124).
- [ ] AC-11: REPL + one-shot session coexistence (§5.2.1) — REPL sets `[sessions].claude = R1`; concurrent `bab -p gemini "hi"` sets `[sessions].gemini = O1`; after both exit, both keys present. Run 10× to catch races.
- [ ] AC-12: One-shot does not start REPL — no readline interface created; no prompt prefix; no history touched. `~/.cache/bab/history` mtime unchanged (or absent — history is in-memory per repl-shell D-03).
- [ ] AC-13: Invalid UTF-8 on stdin (D-02) → `✗ E-CLI-INVALID-UTF8: stdin contains invalid UTF-8 at byte N. bab does not currently support non-UTF-8 prompts.` exits 2. Detection via `Buffer.from(buf).toString('utf8')` followed by re-encode comparison, or via the `'replacement character'` check on the strict decoder.

## §4 Non-Functional Requirements

| Dimension | Requirement |
|-----------|-------------|
| Latency (p95) | One-shot adds ≤ 50 ms exec / ≤ 10 ms ACP overhead (G-07). Cold-start (fresh `node bin/bab.mjs` process) ≤ 80 ms p95 to first byte of provider output, excluding provider compute. Measured in `tests/bench/one_shot.ts` (tinybench). Note: tight under Node startup baseline (~30–60 ms); achievable via lazy imports and `node --no-warnings`. |
| Error budget | Mid-turn crash → `✗ E-CLI-CRASH: provider 'X' crashed (exit N).` to stderr; partial stdout already streamed left intact (not truncated); exits with §4.8-mapped non-zero code (D-01). Never hangs: hard wall-clock cap = `BAB_TURN_TIMEOUT + 2s` after which bab exits `CliTimeout`. |
| Browser support | N/A |
...
> Not applicable — `Applies` does not include `interface-contracts`. The CLI flag grammar is documented in this SPEC's ACs and in `bab --help` (TC-20 snapshot). CLI grammar stability is version-pinned to `bab --version` major-version compatibility.
...
| TC-05 | Integration | `bab -p mock "hi"` with mock streaming 3 chunks | stdout = concatenated chunks; exit 0; stderr empty |
| TC-06 | Integration | `echo "x" \| bab -p mock` | Mock receives prompt "x"; exit 0 |
| TC-07 | Integration | `bab -p mock "pre" < <(printf "post")` | Mock receives `"pre\n\npost"`; exit 0 |
| TC-08 | Integration | `bab -p mock` with stdin as TTY (via node-pty) | stderr `E-CLI-NO-PROMPT`; exit per D-01 NO_PROMPT mapping |
| TC-09 | Integration | `: \| bab -p mock` (empty pipe) | stderr `E-CLI-NO-PROMPT: stdin is empty`; exit non-zero |
| TC-10 | Integration | Env denylist — `ANTHROPIC_API_KEY=leak`, mock dumps env | `ANTHROPIC_API_KEY` absent; `AWS_REGION` present if set |
| TC-11 | Integration | `bab -p mock "hi" \| cat > out.txt; grep -c $'\x1B\\[' out.txt` | Returns 0 |
| TC-12 | Integration | `NO_COLOR=1 bab -p mock "hi"` (PTY harness via node-pty) | stdout contains no ANSI |
| TC-13 | Integration | `FORCE_COLOR=1 bab -p mock "hi" \| cat` | stdout contains ANSI |
| TC-14 | Integration | `bab -p mock-crash "hi"` (mock exits 1 mid-stream) | stderr `E-CLI-CRASH`; exit per D-01; partial stdout preserved |
| TC-15 | Integration | `BAB_TURN_TIMEOUT=1 bab -p mock-hang "hi"` | Exits within 3 s; stderr `E-CLI-TIMEOUT` |
| TC-16 | Integration | `bab -p unknown "x"` | stderr `E-INVALID-PROVIDER`; exit per D-01 |
...
| TC-20 | Snapshot (vitest) | `bab --help` output | `toMatchSnapshot()`; documents `--` separator |
...
- **PRD:** docs/prds/2026-05-23-bab.md §6 — F-07; §4.6, §7.2
...
|---|----------|-------|--------|------------|
| Q-01 | Argument parser — commander vs yargs vs hand-rolled | backend-lead | Resolved (2026-05-23) | `commander` v12 (D-04). ~50 KB packed, well under PRD G-06 install footprint; subcommand support free for future `bab telemetry`/`bab daemon`. |
...
| Q-03 | Should one-shot exit codes be remappable via env (e.g. `BAB_EXIT_CODE_TIMEOUT=99`)? | testing-lead | Open (lean No) | Adds surface area; YAGNI. Resolve at first user request, or 2026-08-15. |
...
- D-01 follows POSIX / GNU / sysexits.h conventions so shell scripts piping bab get predictable signals: `if [ $? -eq 124 ]` works naturally for timeout.
