...
|----|-------------|-------|----------|--------|------------|----------|
| T-01 | Author `package.json`: name `ulm`, MIT+Apache dual license per PRD Q-07, `engines.node: "^20.10 \|\| ^22 \|\| ^24"`, `bin: { ulm: "bin/ulm.mjs" }`, `type: "module"`, `files: ["bin", "README.md", "LICENSE-MIT", "LICENSE-APACHE"]` | devops-lead | High | Done | AC-02, AC-05 | — |
...
| T-04 | Author esbuild config (`scripts/build.mjs` or `esbuild.config.mjs`): bundle `src/index.ts` → `bin/ulm.mjs` as single ESM file with `#!/usr/bin/env node` shebang preserved | devops-lead | High | Done | AC-05 | D-01 |
| T-05 | Add bundle integrity check: `node --check bin/ulm.mjs`; verify no `require()` calls in output | testing-lead | High | Done | TC-03 | — |
...
| T-09 | Add installed-footprint gate: `npm i -g ulm@$PUBLISHED_VERSION` in Ubuntu container, `du -sb $(npm root -g)/ulm` ≤ 8388608 | devops-lead | High | Done | AC-04 | — |
| T-10 | Author cold-install harness `tests/onboarding/cold_install.sh`: fresh runner, `npm i -g ulm && ulm -p stub "ping"` ≤ 120 s; emit JSON timing artifact | testing-lead | High | Done | AC-06 | D-07 |
| T-11 | Author stub provider `tests/onboarding/fixtures/ulm-stub-provider.mjs` (50 LOC) echoing canned response within 50 ms — isolates cold-install gate from provider variance | testing-lead | High | Done | AC-06 | D-07 |
...
| T-17 | Wire `npm publish --provenance --access public` step in release.yml using GitHub Actions OIDC; verify post-publish via `npm view ulm dist.attestations` | security-specialist | High | Done | AC-10 | D-02 |
| T-18 | Author downstream Homebrew formula template at `packaging/homebrew/ulm.rb`: `depends_on "node"`, `install` runs `npm install -g --prefix=#{libexec}`, `bin/ulm` write_env_script shim | devops-lead | High | Done | AC-11 | D-06 |
| T-19 | Wire homebrew-bumper job: on successful npm publish, open PR against `<owner>/homebrew-ulm` updating version + tarball SHA-256 | devops-lead | High | Done | AC-11 | — |
| T-20 | Author AUR PKGBUILD template at `packaging/aur/PKGBUILD`: `depends=('nodejs>=20')`, install into `/usr/lib/node_modules/ulm`, `/usr/bin/ulm` symlinks `node /usr/lib/node_modules/ulm/bin/ulm.mjs` | devops-lead | High | Done | AC-12 | — |
| T-21 | Wire AUR-bumper job: on successful npm publish, open PR against `<owner>/aur-ulm` updating `pkgver` + checksum + `.SRCINFO` | devops-lead | High | Done | AC-12 | — |
| T-22 | Implement cross-platform smoke matrix (9 cells per D-05): Ubuntu 22.04 / 24.04, macOS 13 / 14, Windows 2022 × Node 20.10 / 22-LTS / 24 — `npm i -g ulm@$VERSION && ulm --version` | testing-lead | High | Done | AC-13, TC-05 | D-05 |
...
| T-32 | Write integration test for `npm ci --ignore-scripts` reproducibility: two consecutive runs produce identical `node_modules/ulm/` | testing-lead | High | Done | TC-04 | D-03 |
| T-33 | Write integration test for provenance attestation: `npm view ulm dist.attestations` returns non-empty SLSA L2 metadata | testing-lead | High | Done | TC-08 | D-02 |
| T-34 | Write Homebrew formula syntax test: `brew audit --strict packaging/homebrew/ulm.rb` exits 0 in sandbox | testing-lead | Medium | Done | TC-09 | D-06 |
