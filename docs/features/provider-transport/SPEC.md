...
**Source:** docs/prds/2026-05-23-bab.md §6 — F-02; §5.1, §5.1.1, §5.1.2, §4.7, §4.8, §7.1, §7.2, §9.4
...
- Three transport implementations with per-provider selection per §5.1.1:
  - **exec** — argv-array `child_process.spawn(absolutePath, args, opts)` (D-02 shared `bab-spawn` helper); pipes for stdio
...
- Subprocess lifecycle (§5.1.2): argv-only spawn via shared `src/spawn/index.ts` helper (D-02 — single source of truth); stdout streamed via `readline.createInterface` over child's stdout (UTF-8 line-by-line); stderr buffered, redacted via F-08 on every line (D-09); 60 s exec timeout (`BAB_TURN_TIMEOUT` override) via `AbortController` + `setTimeout`; 30 s no-byte ACP idle kill; pipes (not PTY) for stdio (D-03).
- Cancellation: Ctrl-C → SIGINT (or `tree-kill` on Windows for `CTRL_BREAK_EVENT` semantics) → 500 ms grace → SIGTERM → 500 ms grace → SIGKILL; total ≤ 1.6 s; second Ctrl-C within 1 s exits bab. `AbortController.abort()` signal observed at every stdio read (D-04).
...
- Env denylist applied **before** spawn via shared helper (D-02): `*_API_KEY`, `*_TOKEN`, `*_SECRET`, `*_PASSWORD`, `AWS_*` except `AWS_REGION`, `GOOGLE_APPLICATION_CREDENTIALS`. `BAB_PROPAGATE_ENV` allowlist override.
...
- [ ] AC-04: `BAB_PROPAGATE_ENV="MY_TOKEN,FOO"` allowlists; `MY_TOKEN` and `FOO` reach child; `BAR_TOKEN` doesn't. One log line per session naming propagated vars. `BAB_PROPAGATE_ENV=*` emits a per-turn warning.
- [ ] AC-05: Exec timeout fires at 60 s; `BAB_TURN_TIMEOUT=5` reduces to 5 s. Subprocess receives SIGTERM then SIGKILL per §4.7; child reaped (no zombie — verified by checking `child.exitCode !== null` after `await` resolves).
...
- [ ] AC-11: PID-marker with start-time defeats reuse — fabricate a stale PID-marker for the current shell PID with an old start-time; bab does NOT kill the current shell; stale entry pruned silently.
...
| Spawn timing | Exec: 60 s wall-clock per turn (`BAB_TURN_TIMEOUT` override). ACP: no aggregate cap; 30 s no-byte-received kill; `ping` every 10 s. |
| Cancellation | First Ctrl-C: SIGINT (or Windows `tree-kill` for process-group semantics) → 500 ms → SIGTERM → 500 ms → SIGKILL. Total ≤ 1.6 s. Second Ctrl-C within 1 s exits bab. |
| Browser support | N/A |
| Quota enforcement | bab does not rate-limit (§7.6); provider 429s surface verbatim after credential redaction. |
...
| Concurrency | Exactly one active subprocess per (provider, bab-process). Concurrent `send()` on the same transport instance throw `TransportBusy`. |
...
| TC-15 | Integration | PID-reuse defense | Fake stale PID-marker for current shell PID with fabricated old start-time; bab does NOT kill the current shell |
...
- **PRD:** docs/prds/2026-05-23-bab.md §6 — F-02; §5.1, §5.1.1, §5.1.2, §7.1, §7.2, §9.4
...
| Q-02 | ACP version negotiation — which protocol version does bab advertise? | backend-lead | Open | Resolve alongside PRE-02. Affects AC-07 keepalive format if pre-1.0 ACP diverges. Target 2026-06-13. |
...
- The interface surface is the load-bearing piece for the §9 daemon path. Anything that requires sharing state inside one bab process via module-level globals will need re-architecting when the daemon ships — keep transport state ownership inside the interface impls.
