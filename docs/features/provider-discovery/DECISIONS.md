...
| D-03 | 2026-05-23 | backend-lead | Version-string parsing strategy | Best-effort regex `/(\d+)\.(\d+)\.(\d+)/` per provider; raw-string fallback on no-match | Each provider's `--version` format is unstable; bab must not crash on format change. Raw preserved for forensics; semver computed lazily. | SPEC §10 | — |
