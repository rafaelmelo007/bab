...
| T-05 | Implement `--` separator handling: prove that `bab -p claude -- --help me` treats `"--help me"` as the prompt | backend-lead | High | Done | AC-04 | — |
...
| T-07 | Add `CliNoPrompt` + `CliInvalidUtf8` subclasses to F-08 `BabError` hierarchy (cross-feature dep on F-08) | backend-lead | High | Done | AC-05, AC-06, AC-13 | D-02 |
...
| T-11 | Implement `BabError.exitCode()` table-driven exit per D-01 in `src/oneshot/exitCodes.ts`: 0/1/2/69/74/77/124/126/127 mapping | backend-lead | High | Done | AC-10 | D-01 |
| T-12 | Implement hard wall-clock cap `BAB_TURN_TIMEOUT + 2s` outer timeout (above F-02's per-turn timeout) — never hangs even if F-02 hangs | backend-lead | Medium | Done | (NFR Error budget) | — |
...
| T-20 | Write integration tests for happy paths: `-p mock "hi"` (TC-05), `echo \| bab -p mock` (TC-06), concat (TC-07) | testing-lead | High | Done | TC-05..TC-07 | — |
...
| T-26 | Write integration test for `BAB_TURN_TIMEOUT=1` + hung mock → exit within 3s, `CliTimeout` (TC-15) | testing-lead | High | Done | TC-15 | — |
| T-27 | Write integration test for `bab -p unknown "x"` → `InvalidProvider` exit code (TC-16) | testing-lead | Medium | Done | TC-16, AC-10 | — |
...
| T-30 | Write vitest snapshot test for `bab --help` output (documents `--` separator) | testing-lead | Medium | Done | TC-20 | — |
