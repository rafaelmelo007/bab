...
|----|-------------|-------|----------|--------|------------|----------|
| T-01 | Scaffold `src/errors/index.ts` — declare `BabErrorCode` union + `RenderOpts` + abstract `BabError` per INTERFACE-CONTRACTS.md §1 | backend-lead | High | Done | AC-01..AC-13 | D-08 |
...
| T-03 | Implement `BabError.format(opts)` rendering exact §4.8 templates byte-for-byte; table-driven test fixture per template | backend-lead | High | Done | AC-01..AC-13 | — |
...
| T-07 | Implement `BabError.exitCode()` switch returning per-variant integers per INTERFACE-CONTRACTS.md §3 (consumed by F-07) | backend-lead | High | Done | AC-24 | — |
...
| T-12 | Implement `BAB_REDACT_EXTRA` env parsing: comma-separated regex list, invalid regex warns once and falls back to defaults (additive only per D-03) | security-specialist | High | Done | AC-21 | D-03 |
| T-13 | Wire defense-in-depth: route bab's own stderr (via `console.error` override or `pino` transport) through `scrub()` before write | security-specialist | High | Done | AC-15 (indirectly) | D-05 |
...
| T-17 | Write vitest table-driven test for all 13 BabError variants vs §4.8 templates | testing-lead | High | Done | TC-01 | — |
...
| T-21 | Write integration test spawning `bab -p invalid` etc. via `node:child_process` and asserting `code` callback value matches D-01 table | testing-lead | Medium | Done | TC-10, AC-24 | D-01 |
...
| T-23 | Stub INTERFACE-CONTRACTS export surface (re-export `BabError`, subclasses, `RenderOpts`, `scrub`, `REDACT_VERSION`) — pinned API for F-02/F-05/F-06/F-07 to lock against | prompt-engineer | High | Done | — | D-06 |
